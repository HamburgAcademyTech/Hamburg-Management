...
class FirebaseTeacherRepo : TeacherRepository {
    private val firestore = Firebase.firestore

    override fun getTeachers() = flow {
        firestore.collection("Teachers").snapshots.collect { querySnapshot ->
    ...
    override fun getTeacherById(id: String) = flow {
        firestore.collection("Teachers").document(id).snapshots.collect { documentSnapshot ->
    ...
    override suspend fun updateTeacher(teacher: Teacher) {
        firestore.collection("Teachers").document(teacher.id).set(teacher)
    }

    override suspend fun deleteTeacher(teacher: Teacher) {
        firestore.collection("Teachers").document(teacher.id).delete()
    }
